...
long long int IntensityImage::integrate( Point start, Vector& centroid, Vector& axis1, Vector& axis2, unsigned char oldcol, unsigned char newcol, std::vector<Point>* border ) {
	...
	/*Begin outbound packetstream														*/
	UdpTransmitSocket transmitSocket( IpEndpointName( ADDRESS, PORT ) );
	char buffer[OUTPUT_BUFFER_SIZE];
	osc::OutboundPacketStream p( buffer, OUTPUT_BUFFER_SIZE );

	std::cout << "moments: m00 = " << m.m00  << "oldcol: " << (int)oldcol << "newcol: " << (int)newcol << std::endl;
	if( m.m00 > 45 && oldcol < 250 && newcol !=255 ) {
		p << osc::BeginBundleImmediate << osc::BeginMessage( "/depth" )  << true  << (int)m.m00 << (int)newcol << (int)oldcol << osc::EndMessage << osc::EndBundle;
	...
	/* alternative angle calculation:tar
...
int IntensityImage::threshold( unsigned char value, IntensityImage& target , unsigned char minvalue ) const {
		...
		for (int i = 0; i < count; i++) {
			tmp = ((data[i] > value) && (data[i] <= minvalue)) ? 255 : data[i];
			if (tmp) hits++;
			target.data[i] = tmp;
		}
		return hits;
